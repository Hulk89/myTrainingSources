...
typedef struct ikuRider
{
    ...
    std::vector<ikuInfo>   mAllocInfo;
    int                    mRiderNum;
} ikuRider;

void printRiderResult( ikuRider* aRider )
{
    int sSum = 0;
    int sTotalDelivered = 0;

    for ( std::vector<ikuInfo>::iterator it = aRider->mAllocInfo.begin();
          it != aRider->mAllocInfo.end(); 
          ++it )
    {
        print(*it);
    }
    ...
    cout << "Total delivered : " << sTotalDelivered << endl;
    
    cout << "Rider1 Pos : " << aRider->mPos.x << ", " << aRider->mPos.y << endl;
...
/// sorting
    ...
    ikuRider sRider1[NUM_RIDER];
    ikuRider sRider2[NUM_RIDER];

    // initialize rider
    for ( i = 0 ; i < NUM_RIDER ; i++ )
    {
        sRider1[i].mPos.x = sRider2[i].mPos.x = rand() % MAX_X; 
        sRider1[i].mPos.y = sRider2[i].mPos.y = rand() % MAX_Y; 
        sRider1[i].mRiderNum = 1;
        sRider2[i].mRiderNum = 2;
        
        sRider1[i].mAllocInfo.reserve(NUM_PARCEL);
        sRider2[i].mAllocInfo.reserve(NUM_PARCEL);

        //print riders position
        cout << i << "-th Riders initial position : ( " << sRider1[i].mPos.x 
                                       << ", " << sRider1[i].mPos.y 
                                       << " )" << endl;
    }
...
/* 3. simulate algorithms */
        gTime = i;
        for ( k = 0 ; k < NUM_RIDER ; k++ )
        {
            newAlgorithm( &sCurInfo,
                          &(sRider1[k].mAllocInfo),
                          &(sRider1[k]) );

            oldAlgorithm( &sCurInfo2, 
                          &(sRider2[k].mAllocInfo),
                          &(sRider2[k]) );
        }
    }
/* 4. print Result */
    ...
    for ( k = 0 ; k < NUM_RIDER ; k++ )
    {
        cout << "== " << k << "-th Rider Result ==" << endl;
        printRiderResult( &(sRider1[k]) );
    }
    cout << " ============================================" << endl;
