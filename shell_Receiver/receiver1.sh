...
# receiverServer에 table과 replication 객체를 만든다.

    export ALTIBASE_HOME=${ROOT};
    export ALTIBASE_PORT_NO=${port1};
    export ALTIBASE_REPLICATION_PORT_NO="${replPort}";

    ${ADMIN1} << EOF
CREATE TABLE TABLE${index} ( I1 INTEGER PRIMARY KEY, I2 INTEGER );
CREATE REPLICATION REP${index} WITH '127.0.0.1', ${replPortNo}
FROM SYS.TABLE${index} TO SYS.TABLE${index};
quit
EOF
...
# ActiveServer에 table과 replication 객체를 만들고 replication을 start한다.

    ${ADMIN1} << EOF
CREATE TABLE TABLE${index} ( I1 INTEGER PRIMARY KEY, I2 INTEGER );
CREATE REPLICATION REP${index} WITH '127.0.0.1', ${replPort}
FROM SYS.TABLE${index} TO SYS.TABLE${index};
ALTER REPLICATION REP${index} START;
quit
EOF
